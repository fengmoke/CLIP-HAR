...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str, default='TS2ACT-main/dataset/data',
                        help="path to Dataset")
    parser.add_argument("--checkpoint_root", type=str, default='output',
                        help="path to checkpoint")
    parser.add_argument('--conti', type=int, help='Continue training from last checkpoint', default=0)
    dataset_name = 'PAMAP2'
    parser.add_argument("--dataset", type=str, default= dataset_name,
                        choices=['UCI','UCI2','PAMAP2','MotionSense', 'WISDM'], help='Name of dataset')

    parser.add_argument('--shot', type=str,
                        default='orgin_dataset')
    ...
    if dataset_name in ['UCI']: 
        ...
        parser.add_argument("--batch_size", type=int, default=64)
        parser.add_argument("--val_batch_size", type=int, default=64)
        parser.add_argument("--resolution", type=int, default=171)
...
def train_model(opt, model, optimizer, scheduler, labeled_dst , unlabeled_tra=None, unlabeled_val=None,best_acc = 0.0):
    ...
    for epoch in range(Epochs):
        # epoch_bar.desc = "lr: {}".format(optimizer.state_dict()['param_groups'][0]['lr'])
        ...
        batch_idx_bar = tqdm(range(100), file=sys.stdout)
        for batch_idx in batch_idx_bar:
            try:
                inputs_x, y = next(labeled_iter)
            except:
                labeled_iter = iter(labeled_dataloader)
                inputs_x, y = next(labeled_iter)
            try:
                inputs_u_w, inputs_u_s, y_u = next(unlabeled_iter)
            except:
                unlabeled_iter = iter(unlabeled_dataloader)
                inputs_u_w, inputs_u_s, y_u = next(unlabeled_iter)
            optimizer.zero_grad()
...
def init_dataloader(opt):
    ...
    if opt.dataset == 'PAMAP2':
        labeled_dst ,unlabeled_tra, test_dst = PAMAP_TS2ACT_aug(clip_len=opt.clip_len,name = opt.shot)
    if opt.dataset == 'HHAR':
        labeled_dst ,unlabeled_tra = HHAR_TS2ACT_aug(clip_len=opt.clip_len,name = opt.shot)
    if opt.dataset == 'UCI':
        labeled_dst ,unlabeled_tra, test_dst = UCI_TS2ACT_aug(clip_len=opt.clip_len,name = opt.shot)
    c = None
    return labeled_dst ,unlabeled_tra, test_dst
...
def main(opt):
    ...
    model = TimeTransformer(opt.resolution, opt.num_feature, 512 ,layers= 6,heads=16, output_dim=opt.num_class, dropout=0.0)
