...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default= dataset_name,
                        choices=['UCI', 'PAMAP2','MotionSense'], help='Name of dataset')
...
def init_dataloader(opt):
    if opt.dataset == 'MotionSense':
        labeled_dst ,unlabeled_tra = MotionSense_aug(clip_len=opt.clip_len, name = opt.shot)
    if opt.dataset == 'PAMAP2':
        labeled_dst ,unlabeled_tra, test_dst = PAMAP2_aug(clip_len=opt.clip_len,name = opt.shot)
    if opt.dataset == 'UCI':
        labeled_dst ,unlabeled_tra, test_dst = UCI_aug(clip_len=opt.clip_len,name = opt.shot)
    c = None
